...
const SupportManagement: React.FC = () => {
  ...
  return (
    <Box>
      <PageHeader title="지원 티켓 관리" />

      {/* 통계 카드 */}
      {stats && (
        <Grid container spacing={2} sx={{ mb: 4 }}>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <StatCard label="접수 대기" value={stats.open_count} icon={<SupportAgentIcon />} color="error" />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <StatCard label="처리중" value={stats.in_progress} icon={<HourglassEmptyIcon />} color="warning" />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <StatCard label="해결 완료" value={stats.resolved} icon={<CheckCircleIcon />} color="success" />
          </Grid>
          <Grid size={{ xs: 12, sm: 6, md: 3 }}>
            <StatCard label="종료" value={stats.closed} icon={<ArchiveIcon />} color="info" />
          </Grid>
        </Grid>
      )}
      ...
      {loading ? (
        ...
        <EmptyState icon={<SupportAgentIcon />} title="티켓이 없습니다." />
      ) : (
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ display: { xs: 'none', md: 'table-cell' } }}>접수일</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {tickets.map((ticket) => (
                ...
                >
                  <TableCell sx={{ display: { xs: 'none', md: 'table-cell' } }}>{ticket.id}</TableCell>
                  <TableCell>
                    <Box>
                  ...
                  <TableCell sx={{ display: { xs: 'none', md: 'table-cell' } }}>{formatDateTime(ticket.created_at)}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
